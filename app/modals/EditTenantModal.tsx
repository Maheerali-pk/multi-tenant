...
import type { TenantRow } from "@/app/components/TenantsTable";
...
export default function EditTenantModal({
  ...
  // Fetch tenant data when modal opens
  useEffect(() => {
    if (isOpen && tenant) {
      fetchTenantData();
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
      <div className="bg-bg-inner rounded-3xl p-6 shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-text-dark">Edit Tenant</h2>
          <button
            onClick={onClose}
            disabled={loading}
