...
import type { AssessmentItemRow } from "./assessment-items-table";
...
type AssessmentItemUpdate = TablesUpdate<"assessment_items">;

interface EditAssessmentItemModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
...
export default function EditAssessmentItemModal({
  isOpen,
  onClose,
  onSuccess,
  item,
}: EditAssessmentItemModalProps) {
  ...
  const loadItemData = async () => {
    ...
    try {
      ...
      if (itemError) {
        console.error("Error fetching assessment item:", itemError);
        setError("Failed to load assessment item data");
      ...
      if (itemData) {
        setFormData({
          ...
          what_to_check: itemData.what_to_check || "",
          category: itemData.category || "",
          how_to_check: itemData.how_to_check || "",
          check_method: itemData.check_method || "",
          evidence_hint: itemData.evidence_hint || "",
          how_it_helps: itemData.how_it_helps || "",
          importance: itemData.importance || "",
          required: itemData.required || false,
          is_active: itemData.is_active ?? true,
          sort_order: itemData.sort_order || 0,
          integration_key: itemData.integration_key || "",
          notes: itemData.notes || "",
        });
      }
    } catch (err) {
      console.error("Unexpected error loading assessment item:", err);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!item) {
      setError("Assessment item not found");
      return;
    }
    ...
    try {
      const itemData: AssessmentItemUpdate = {
      ...
      if (updateError) {
        ...
        setError(updateError.message || "Failed to update assessment item");
        setLoading(false);
        return;
      }

      // Show success toast
      toast.success("Assessment item updated successfully");
